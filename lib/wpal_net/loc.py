...
def localize(net, db, output_dir, ave, sigma, dweight, attr_id=-1, vis=False, save_dir=None):
    ...
    for i in db.test_ind:
        ...
        def find_heat(bin_ind):
            return heat[bin_ind / num_bin_per_detector[find_layer_ind(bin_ind)]]

        # find the target a bin detects.
        # TODO: check the target location against the bin's region
        def find_target(bin_ind):
            locs = np.where(find_heat(bin_ind) == score[bin_ind])
            if len(locs[0]) > 1:
        ...
        for a in attr_list:
            ...
            y = sum([w_func(j) / w_sum * target[j][0] / np.array(find_heat(j)).shape[0]
                     for j in xrange(len(score))])
            x = sum([w_func(j) / w_sum * target[j][1] / np.array(find_heat(j)).shape[1]
                     for j in xrange(len(score))])
            # Superposition of the heat maps.
            superposition = sum([cv2.resize(w_func(j) / w_sum * find_heat(j)
                                            * gaussian_filter(find_heat(j).shape,
                                                              y * find_heat(j).shape[0],
                                                              x * find_heat(j).shape[1],
                                                              img_area / find_heat(j).shape[0] * find_heat(j).shape[1]),
                                            (w, h))
                                 for j in xrange(len(score))])
            ...
            for j in [_[0] for _ in sorted(enumerate([w_func(k) for k in xrange(len(score))]),
                ...
                val_scale = 255.0 / max(max(__) for __ in find_heat(j))
                canvas = np.zeros_like(img)
                canvas[..., 2] = cv2.resize((find_heat(j) * val_scale).astype('uint8'), (img.shape[1], img.shape[0]))
                _y = 1.0 * target[j][0] / np.array(find_heat(j)).shape[0]
                _x = 1.0 * target[j][1] / np.array(find_heat(j)).shape[1]
